npm-serverless-dynamodb-local
=================================

[![Join the chat at https://gitter.im/99xt/npm-serverless-dynamodb-local](https://badges.gitter.im/99xt/npm-serverless-dynamodb-local.svg)](https://gitter.im/99xt/npm-serverless-dynamodb-local?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)
[![npm version](https://badge.fury.io/js/npm-serverless-dynamodb-local.svg)](https://badge.fury.io/js/npm-serverless-dynamodb-local)
[![license](https://img.shields.io/npm/l/npm-serverless-dynamodb-local.svg)](https://www.npmjs.com/package/npm-serverless-dynamodb-local)
...
## Install Plugin
`npm install --save npm-serverless-dynamodb-local`

Then in `serverless.yml` add following entry to the plugins array: `npm-serverless-dynamodb-local`
```yml
plugins:
  - npm-serverless-dynamodb-local
```
...
Make sure that `npm-serverless-dynamodb-local` is above `serverless-offline` so it will be loaded earlier.

Now your local DynamoDB database will be automatically started before running `serverless offline`.

### Using with serverless-offline and serverless-webpack plugin
Run `serverless offline start`. In comparison with `serverless offline`, the `start` command will fire an `init` and a `end` lifecycle hook which is needed for serverless-offline and npm-serverless-dynamodb-local to switch off both ressources. 
...
* [NPM Registry](https://www.npmjs.com/package/npm-serverless-dynamodb-local)
